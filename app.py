...
def home():
    if request.method == 'POST':
        if "play" in request.form:
            # GPT-3にトピックを考えさせる
            chat = openai.ChatCompletion.create(model="gpt-3.5-turbo-0301", messages=[{"role": "system", "content": "あなたは私の20問ゲームの対戦相手です。今から1つの答えを考えてください。"}, {"role": "user", "content": "20問ゲームにあう答えを１つ決めてください。答えの単語だけ返事してください。"}])
            ...
            if 'topic' in session:
                ...
                # GPT-3に質問を評価させる
                chat = openai.ChatCompletion.create(model="gpt-3.5-turbo-0301", messages=[{"role": "system", "content": "あなたは私の20問ゲームの対戦相手です。「はい」「少しそう」「どちらでもない」「違います」「少し違う」のいずれかだけで返事をします。"}, {"role": "user", "content": f'答えは"{session["topic"]}"です。私は答えに対して"{question}"と質問しました。質問に対して「はい」「少しそう」「どちらでもない」「違います」「少し違う」のいずれかだけで返事してください。'}])
