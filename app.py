...
def home():
    if request.method == 'POST':
        if "play" in request.form:
            # GPT-3にトピックを考えさせる
            chat = openai.ChatCompletion.create(model="gpt-4", messages=[{"role": "system", "content": "あなたは私の20問ゲームの対戦相手です。今から1つの答えを考えてください。"}, {"role": "user", "content": "20問ゲームに適切な答えを1つ決めてください。答えは単語でなければなりません。"}])
            ...
            if 'topic' in session:
                ...
                # GPT-3に質問を評価させる
                chat = openai.ChatCompletion.create(model="gpt-4", messages=[{"role": "system", "content": "あなたは私の20問ゲームの対戦相手です。「はい」「少しそう」「どちらでもない」「違います」「少し違う」のいずれかだけで返事をします。"}, {"role": "user", "content": f'私が答えに"{session["topic"]}"を選んでいます。私は答えに対して"{question}"と質問しました。質問に対して「はい」「少しそう」「どちらでもない」「違います」「少し違う」のいずれかだけで返事してください。'}])
